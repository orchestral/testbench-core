...
trait InteractsWithWorkbench
{
    ...
    protected function getPackageBootstrappersUsingWorkbench($app)
    {
        if (
            ! $this->isRunningTestCase()
            || empty($bootstrappers = (optional(static::$cachedConfigurationForWorkbench)->getExtraAttributes()['bootstrappers'] ?? null))
        ) {
            return null;
        }
    ...
    protected function getPackageProvidersUsingWorkbench($app)
    {
        if (
            ! $this->isRunningTestCase()
            || empty($providers = (optional(static::$cachedConfigurationForWorkbench)->getExtraAttributes()['providers'] ?? null))
        ) {
            return null;
        }
